			 ...
			 //execute function to remove lines
				 ...
				 menu_2 = '0';
					...
					if (choose >= 0 && choose < dim_line){
						...
						printf("Station ID: ");
						scanf("	%[^\n]s", id_estacao);
						for (int i = 0; i < strlen(id_estacao); i++){
							id_estacao[i] = (char)toupper(id_estacao[i]);
						}
						for (int i = 0; i < dimensition; i++){
							if(strcmp(saved_stations[i]->id, id_estacao) == 0){
								add_line(line[choose], saved_stations[i]->station_name, saved_stations[i]->id, 0);
